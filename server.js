  ...
  try {
    ...
    try {
      ...
      Object.entries(drawsByDate).forEach(([fecha, draws]) => {
        // Ordenar draws por hora ascendente (el más antiguo primero) para hallar el primer sorteo del día
        ...
        // Obtener los 120 sorteos anteriores al primer sorteo de este día
        ...
        if (prevDraws.length >= 15) {
          totalPossibleHits += 5; // 5 animales recomendados por día en cada lista
          ...
          // Cargar pesos correspondientes a la fecha (usando el primer sorteo)
          ...
          // Ganadores reales de ese día
          const dailyWinners = draws.map(s => obtenerCodigoResultado(s.resultado)).filter(Boolean);

          // Sumar aciertos únicos de los 5 recomendados en el día
          const hotHits = dailyCalientes.filter(num => dailyWinners.includes(num)).length;
          const coldHits = dailyAtrasados.filter(num => dailyWinners.includes(num)).length;
          const fijosHits = dailyFijos.filter(num => dailyWinners.includes(num)).length;

          sumHotHits += hotHits;
          sumColdHits += coldHits;
          sumFijosHits += fijosHits;
        }
      });

      aciertoCalientesSemana = totalPossibleHits > 0 ? Math.round((sumHotHits / totalPossibleHits) * 100) : 0;
      aciertoAtrasadosSemana = totalPossibleHits > 0 ? Math.round((sumColdHits / totalPossibleHits) * 100) : 0;
      aciertoFijosSemana = totalPossibleHits > 0 ? Math.round((sumFijosHits / totalPossibleHits) * 100) : 0;
    } catch (backtestErr) {
...
app.get('/api/configuracion/riesgos/export-xls', async (req, res) => {
  ...
  try {
    ...
    sortedDates.forEach(fecha => {
      ...
      if (prevDraws.length >= 15) {
        ...
        sortedDraws.forEach(s => {
          ...
          const resultName = targetLoteriaMap[resultNum] || 'Desconocido';
          const resultText = `${resultName} #${resultNum}`;
          ...
          const formatList = (list) => list.map(num => `${targetLoteriaMap[num] || 'Desconocido'} #${num}`).join(', ');

          detailRows.push({
            Fecha: s.fecha,
            Hora: s.hora,
            "Resultado Ganador": resultText,
            "Recomendados Calientes": formatList(dailyCalientes),
            "Acierto Caliente": isHotHit ? "SÍ" : "NO",
            "Fijos del Día": formatList(dailyFijos),
            "Acierto Fijo": isFijosHit ? "SÍ" : "NO",
            "Fríos / Atrasados": formatList(dailyAtrasados),
            "Acierto Frío": isColdHit ? "SÍ" : "NO"
          });
          ...
          if (!hourlyStats[s.hora]) {
            hourlyStats[s.hora] = {
              Hora: s.hora,
              "Sorteos Evaluados": 0,
              "Aciertos Calientes": 0,
              "Aciertos Fijos": 0,
              "Aciertos Fríos": 0
            };
          }

          hourlyStats[s.hora]["Sorteos Evaluados"]++;
          if (isHotHit) hourlyStats[s.hora]["Aciertos Calientes"]++;
          if (isFijosHit) hourlyStats[s.hora]["Aciertos Fijos"]++;
          if (isColdHit) hourlyStats[s.hora]["Aciertos Fríos"]++;
        });
      }
    });

    // Formatear resumen por hora
    const hourlyRows = Object.values(hourlyStats).map(h => {
      const total = h["Sorteos Evaluados"] || 1;
      return {
        Hora: h.Hora,
        "Sorteos Evaluados": h["Sorteos Evaluados"],
        "Aciertos Calientes": h["Aciertos Calientes"],
        "% Eficiencia Calientes": `${Math.round((h["Aciertos Calientes"] / total) * 100)}%`,
        "Aciertos Fijos": h["Aciertos Fijos"],
        "% Eficiencia Fijos": `${Math.round((h["Aciertos Fijos"] / total) * 100)}%`,
        "Aciertos Fríos": h["Aciertos Fríos"],
        "% Eficiencia Fríos": `${Math.round((h["Aciertos Fríos"] / total) * 100)}%`
      };
    ...
    autofitColumns(wsHourly);
    autofitColumns(wsDetail);

    XLSX.utils.book_append_sheet(wb, wsHourly, "Resumen por Hora");
    XLSX.utils.book_append_sheet(wb, wsDetail, "Detalle de Aciertos");

    // Escribir a un buffer binario
    const buffer = XLSX.write(wb, { type: "buffer", bookType: "xlsx" });

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="auditoria_riesgos_${normLotId}.xlsx"`);
    res.send(buffer);

  } catch (err) {
    console.error("Error al exportar Excel de riesgos:", err);
